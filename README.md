...
| **Notes** | Notion-style notebook with a full-width home page (hero, search bar, recent pages list) and a 3-panel editor view (left nav sidebar with page navigation, middle pages list with time grouping: Today / Yesterday / This Week / Earlier, right editor panel with ribbon toolbar). Ribbon includes bold, italic, underline, strikethrough, font size, text colour, highlight, H1/H2/H3, alignment, indent/outdent, bullet/numbered/checklist, links, HR, and code block. Pin notes, search across content, draw with a stylus or finger. Resizable left/right sidebars (drag handle or double-click to collapse). Right sidebar shows live Outline (click heading to scroll) and Page Info (words, characters, reading time). |
